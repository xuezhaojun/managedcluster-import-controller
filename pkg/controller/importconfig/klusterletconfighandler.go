...
func (e *enqueueManagedClusterByBootstrapKubeConfigSecrets) Create(ctx context.Context, evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	e.enqueue(evt.Object.GetName(), q)
}

func (e *enqueueManagedClusterByBootstrapKubeConfigSecrets) Update(ctx context.Context, evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	e.enqueue(evt.ObjectNew.GetName(), q)
}

func (e *enqueueManagedClusterByBootstrapKubeConfigSecrets) Delete(ctx context.Context, evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	e.enqueue(evt.Object.GetName(), q)
}

func (e *enqueueManagedClusterByBootstrapKubeConfigSecrets) Generic(ctx context.Context, evt event.GenericEvent, q workqueue.RateLimitingInterface) {
	e.enqueue(evt.Object.GetName(), q)
}

func (e *enqueueManagedClusterByBootstrapKubeConfigSecrets) enqueue(secretName string, q workqueue.RateLimitingInterface) {
	...
	for _, kcObj := range klusterletconfigObjs {
		...
		managedclusterObjs, err := e.managedclusterIndexer.ByIndex(ManagedClusterKlusterletConfigAnnotationIndexKey, kc.GetName())
		if err != nil {
			klog.Error(err, "Failed to get managedclusters by klusterletconfig annotation by indexer", "klusterletconfig", kc.GetName())
			return
		}
