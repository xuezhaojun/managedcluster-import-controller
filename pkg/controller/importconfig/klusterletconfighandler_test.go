...
func TestEnqueueManagedClusterInKlusterletConfigAnnotation(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			// create a klusterletconfig, expect the managed cluster to be enqueued
		...
		{
			// delete a klusterletconfig, expect the managed cluster to be enqueued
			addEvent: func(h handler.EventHandler, queue workqueue.RateLimitingInterface) {
				// Create a new create event for the managed cluster
		...
		{
			// create a global klusterletconfig, expect all managed clusters to be enqueued
			addEvent: func(h handler.EventHandler, queue workqueue.RateLimitingInterface) {
				// Create a new create event for the managed cluster
				evt := event.CreateEvent{
					Object: &klusterletconfigv1alpha1.KlusterletConfig{
	...
	testcases := []struct {
		...
		},
		{
			...
			verify: func(t *testing.T, queue workqueue.RateLimitingInterface) {
				...
				item, _ := queue.Get()
				if item.(reconcile.Request).Name != "test2" &&
					item.(reconcile.Request).Name != "test3" {
					t.Errorf("Expected item to be test2 or test3, but got %v", item.(reconcile.Request).Name)
				}
